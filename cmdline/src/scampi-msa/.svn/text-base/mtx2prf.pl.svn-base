...
$mtx_file=$ARGV[0] || die("$0: Need an argument");
($mtx_file =~ /^((.+\/)?(.+))\.mtx$/) || die("Input file has faulty name ('$mtx_file')");
$prf_file = $2 . $3 . ".prf";
$seq_id=$3;
...
open(IN,$mtx_file) || die("Couldn't open $mtx_file");
open(OUT,">$prf_file") || die("Couldn't open $prf_file for writing");
$length=<IN>;
$seq=<IN>;
for ($i=0;$i<12;$i++) { <IN>; }
$lambda=0.3176;
...
$col=1;
while(<IN>) {
    /^((\d|-)+\s+){26,28}$/ || die("Faulty format in $mtx_file, faulty line: '$_'");
    @log_list=split(/\s+/,$_);
