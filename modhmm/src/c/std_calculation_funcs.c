...
long double get_user_defined_emission_score(char label, struct letter_s *seq, int seq_pos)
{
//printf("get_user_defined_emission_score\n");
...
const long double bgr_freq[] = { 0.0785,    //A
  ...
  const long double RETURN_G = 0.05;
  const long double RETURN_J = 0.074;
  //const long double RETURN_J = 0.07;
  const long double DG2PROB = 0.582;
  const long double DG_SHIFT_FIRST = -1.60;
  const long double DG_SHIFT_MIDDLE = 1.00;
  const long double DG_SHIFT_LAST = -1.40;
  //const long double DG_SHIFT = 0.8;

  long double DG, DG_sum=0, DG_sin_sum=0, DG_cos_sum=0, segment_DG, prob;
  int helix_length = 21; //iHelixLength;
  //printf("HelixLength: %i\n", iHelixLength);
  //int helix_length=19;
  //printf("%c\n", ((seq + seq_pos)->letter)[0]);
  //printf("%c\n", label);
  //printf("%d\n", (int) (((seq + seq_pos)->letter)[0]-65));
  ...
  // End position of the membrane region of middle membranes
else if (label >= 'a' && label <= 'z') {
    int i, j=1;
    if(label <= 'g') {
      helix_length = label - 80;
      //printf("label: %c,  helix_length = %d,  seqPost: %d\n", label, helix_length, seq_pos);
    }
    else {
      helix_length = label - 87;
     //printf("label: %c,  helix_length = %d,  seqPost: %d\n", label, helix_length, seq_pos);
    }
   if (label == 'x' || label == 'y' || label=='a' || label=='b' || label=="e"|| label=="l")
   {
    helix_length = 21;
   }
    ...
    for (i=seq_pos-helix_length+1;i<=seq_pos;i++) {
      //printf("label: %c,  helix_length = %d,  seqPost: %d, j: %d\n", label, helix_length, seq_pos,j);
      DG = pos_spec_DG(((seq+i)->letter)[0],j,helix_length);
      DG_sum += DG;
      DG_sin_sum += (DG * sin(100*(j-1)*pi_over_180));
      DG_cos_sum += (DG * cos(100*(j-1)*pi_over_180));
    ...
    if (label == 'x'|| label=='y')
    {
      prob = 1 - 1 / ( 1 + exp( -1 * (segment_DG - DG_SHIFT_FIRST) / DG2PROB ) );
    }
    else
    {
        if (label == 'a'|| label=='b'){
            prob = 1 - 1 / ( 1 + exp( -1 * (segment_DG - DG_SHIFT_LAST) / DG2PROB ) );
        }
        else{
            prob = 1 - 1 / ( 1 + exp( -1 * (segment_DG - DG_SHIFT_MIDDLE) / DG2PROB ) );
        }
    }
    //printf("pos = %d: prob = %Lf, segment_DG = %Lf\n", seq_pos, prob, segment_DG);
    return prob;
...
long double get_user_defined_emission_score_msa(int a_size, char label, struct msa_letter_s *msa_seq, int seq_pos)
{
  ...
  const long double RETURN_J = 0.06;
  const long double DG2PROB = 0.582;
  //const long double DG_SHIFT = 1.1;
  const int MAX_MEM_KR = 1;

  const long double DG_SHIFT_FIRST = -1.80;
  const long double DG_SHIFT_MIDDLE = 1.30;
  const long double DG_SHIFT_LAST = -1.50;
